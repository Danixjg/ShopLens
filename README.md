...
**The submission is complete and frozen.** Configuration `O` is what the
official harness grades, it is what a bare `Agent()` builds with no environment
variables set, and both splits are measured and recorded in `results.jsonl`.
The agent runs end to end, offline, over all 200 public sessions, and every
number quoted on this page comes from that evidence log.

The remaining letters are research, not unfinished submission work. `G` and `H`
name optional components the plan never specifies, and `O+`, `L`, `AA`, `W`,
`X`, `Y` and `J` are runnable variants that were not retained. None of them is
reachable from the graded path. [Limitations](#limitations) sets out the honest
edges of what does ship.
