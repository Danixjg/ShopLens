...
|---|---|---|
| Adopt | Question redundancy is a measured failure, not a prompt bug | The paper reports question self-similarity climbing across turns despite an explicit no-duplicates instruction. ShopLens already refuses repeated open questions and covered slots; extend that with an asked-aspect diversity guard behind a named ablation, tuned dev-only. |
| Adopt | Filter clarification aspects by field population | The paper cannot answer questions about legitimately empty fields. ShopLens has the same shape of problem in its own catalog, where color is absent from more than half of products. Gate facet eligibility on field population before asking. |
