...
This record governs how TrippyShoppy may use ideas from *ProductAgent*. It
...
TrippyShoppy cites, and the difference changes what is permitted.
...
| Reported metric values | Quote only here | The paper numbers were measured on AliMe KG with an LLM agent and simulator. They are evidence about ProductAgent, not about TrippyShoppy, and must never appear on a TrippyShoppy results surface. |
...
TrippyShoppy is deterministic, offline, and bound to a frozen 50,000-row catalog
...
| Evaluate | Fusion and reranking can degrade a strong lexical baseline | TrippyShoppy has independent evidence pointing the same way, since dense fusion alone regressed against the lexical baseline. Record it as a consistency check; it justifies no new change on its own. |
| Evaluate | Progressive constraint relaxation when a query returns nothing | TrippyShoppy already scores constraints as penalties rather than filters and relaxes to category before a global fallback. Compare behavior against the published description before adding any mechanism. |
...
persuasion. TrippyShoppy performs no cross-session profiling, stores no user
...
as a reference to TrippyShoppy documentation, because it belongs to an external
document series.
